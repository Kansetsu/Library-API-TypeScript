...
async function libraryControllerPlugin(router, opts) {
    router.post("/library/create", async (request, reply) => {
    ...
    router.get("/library/book", async (request, reply) => {
        ...
        reply.send(await service.getBookByName(request.query));
    });
    router.get("/library/paginate", async (request, reply) => {
        // #swagger.tags = ['Library']
    ...
    router.get("/library/author", async (request, reply) => {
        ...
        reply.send(await service.getBookByAuthor(request.query));
    });
    router.get("/library/category", async (request, reply) => {
        // #swagger.tags = ['Library']
        // #swagger.summary = 'Get a book by category and author '
        // #swagger.description = 'Get a book by category and author'
        // #swagger.parameters['category'] = { description: 'Book category'}
        // #swagger.parameters['author'] = { description: 'Author name'}
        reply.send(await service.getBookByCategory(request.query));
    });
    router.get("/library/category/:category", async (request, reply) => {
        // #swagger.tags = ['Library']
        // #swagger.summary = 'Get a book by category and paginate'
        // #swagger.description = 'Get a book by category and paginate'
        // #swagger.parameters['category'] = { description: 'Book category'}
        // #swagger.parameters['page'] = { description: 'Page number', required: true }
        // #swagger.parameters['books'] = { description: 'Number of books per page', required: true}
        reply.send(await service.getBookByCategoryAndPaginate(request.params, request.query));
    });
    ...
    router.delete("/library/delete", async (request, reply) => {
